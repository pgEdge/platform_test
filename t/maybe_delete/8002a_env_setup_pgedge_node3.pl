# This test case will create the necessary setup of node3 of a three-node cluster.
# It will check for the existance of n1 directory which if exists, its nodectl installed 
# setup would be copied in n3 rather than having to download/install it again from repo. 
# If the n1 directory does not exist, it will perform a new download from the repo. 

use strict;
use warnings;
use File::Which;
use IPC::Cmd qw(run);
use Try::Tiny;
use JSON;
use File::Copy;
use lib './t/lib';
use contains;
use edge;


my $n1dir = "$ENV{EDGE_CLUSTER_DIR}/n1";
my $homedir1 = "$n1dir/pgedge";
my $n3dir = "$ENV{EDGE_CLUSTER_DIR}/n3";
my $homedir3 = "$n3dir/pgedge";
my $ncdir = "$ENV{NC_DIR}";
my $cli = "$ENV{EDGE_CLI}";
my $pgversion = "$ENV{EDGE_COMPONENT}";
my $exitcode = 0;
my $skipInstall = 0;
my $ncdir_copy = "/tmp/nccopy";


# Check if $n3dir is already present
if (-e $n3dir) {
    print("$n3dir already exists\n");
    # check for the minimum existance of install.py and the extracted pgedge directory
    if (-e "$n3dir/install.py" && -d "$n3dir/pgedge") {
        print "nodectl already installed in $n3dir , exiting with success\n";
        $exitcode = 0;
        $skipInstall = 1; #setting it to 1 for the if condition to equate to false in the download and install section
    }
}
else {

    # Check if $n1dir exists
    if (-e $n1dir) {
        # Copy n1dir to n3dir 
        # Also creating $n3dir
        print("Creating $n3dir\n");
        run_command_and_exit_iferr(qq(mkdir -p $n3dir));

        print("copying $n1dir contents to $n3dir\n");
        # Moving $ncdir to $n3dir (to avoid a multi step copy and delete for delete/cleanup)
        #run_command_and_exit_iferr(qq(mv $ncdir_copy/* $n3dir/ && rm -rf $ncdir_copy));

        run_command_and_exit_iferr(qq(cp -r -T $n1dir/. $n3dir/));

        print "Contents copied from $n1dir to $n3dir\n";
        $skipInstall = 1;
    }
    else {
        print ("$n1dir does not exist, proceeding to download and install pgedge");
        $skipInstall = 0;
    }
}

# skip the curl download and python install.py if its been copied from pre-downloaded nc directory 
if (!$skipInstall)
{
    # Create $n3dir with -p switch
    print("Creating $n3dir\n");
    run_command_and_exit_iferr(qq(mkdir -p $n3dir));

    # Download the install.py file into the directory
    print("Download the install.py file into the directory\n");
    run_command_and_exit_iferr(qq(curl -fsSL $ENV{EDGE_REPO} > $n3dir/install.py));

    # Move into the pgedge directory and run the install.py file
    print ("Move into the pgedge directory and run the install.py file\n");
    chdir("./$n3dir");
    run_command_and_exit_iferr(qq(python install.py));
    chdir("./../../../../");
}

exit ($exitcode);
